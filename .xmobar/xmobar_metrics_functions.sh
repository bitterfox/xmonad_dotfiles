...
volume() {
    ...
    if [ -n "`echo $name` | grep hdmi-stereo" ]; then
        # alsa_output.pci-0000_00_1f.3.hdmi-stereo-extra1
        name="HDMI"
    elif [ -n "`echo $name` | grep analog-stereo.equalizer" ]; then
        # alsa_output.pci-0000_00_1f.3.analog-stereo.equalizer
        name="EQUA"
    elif [ -n "`echo $name` | grep analog-stereo" ]; then
