...
configureMouse config = do
  let gamemode_path = "/tmp/xmonad_game_mode"
  gamemode_exists <- liftIO $ doesFileExist gamemode_path
  if gamemode_exists then do
      gamemode <- liftIO $ fmap (head . lines) $ readFile gamemode_path
      if gamemode == "on" then return config
      else configureMouse' config
  else configureMouse' config

configureMouse' config = return $ config {
