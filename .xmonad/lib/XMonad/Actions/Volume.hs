...
amixerSet v m s = outputOf $ "amixer -c 1 set '" ++ s ++ "' " ++ show (clip v) ++ "% " ++ (if m then "" else "un") ++ "mute"
amixerGetAll    = fmap parseAmixerGetAll . mapM amixerGet
amixerGet     s = outputOf $ "amixer -c 1 get \'" ++ s ++ "\'"
...
amixerSetVolumeOnly v s = outputOf $ "amixer -c 1 set '" ++ s ++ "' " ++ show (clip v) ++ "% "

amixerSetMuteOnlyAll :: Bool -> [String] -> IO ()
amixerSetMuteOnly    :: Bool ->  String  -> IO String
amixerSetMuteOnlyAll  = mapM_ . amixerSetMuteOnly
amixerSetMuteOnly m s = outputOf $ "amixer -c 1 set '" ++ s ++ "' " ++ (if m then "" else "un") ++ "mute"
