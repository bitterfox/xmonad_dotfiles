...
module XMonad.Util.NamedScratchpad2 (
  -- * Usage
  -- $usage
  NamedScratchpad(..),
...
registerNSP :: NamedScratchpad -> X ()
registerNSP nsp = do
    NSPState exs scratches <- XS.get
    if null scratches then do
        fillNSPState $ [nsp]
        return ()
    else do
      let nsps = (Map.elems $ Map.delete (name nsp) scratches) ++ [nsp]
      let nspState = NSPState (fillOut exs nsps) $ nspScratches nsps
      XS.put nspState
  where
    nspScratches :: [NamedScratchpad] -> Map String NamedScratchpad
    nspScratches nsps = Map.fromList $ zip (map name nsps) nsps
    fillOut :: Map String [NamedScratchpad] -> [NamedScratchpad] -> Map String [NamedScratchpad]
    fillOut exs nsps = foldl' (\nspMap n -> Map.map (replaceWith n) nspMap) exs nsps
    replaceWith :: NamedScratchpad -> [NamedScratchpad] -> [NamedScratchpad]
    replaceWith n = map (\x -> if name x == name n then n else x)
